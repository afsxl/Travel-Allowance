    ...
    if request.method == "POST":
        ...
        if source_id and destination_id:
            ...
            return render(
                request,
                "add_route.html",
                {"route_stops": route_stops, "success": "Route added successfully"},
            )
...
def add_route_links(request, route_id):
    ...
    return render(
        ...
        {"route": route, "stops": stops, "last_stop": last_stop, "modes": modes},
    )
...
def add_route_path(request, route_id):
    ...
    intermediate_links = RouteLink.objects.filter(route=route).order_by(
        "id"
    )  # Links in sequence
...
def add_route_stop(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect(
                "add_route_stop"
            )  # Or wherever you want the user to go after submission
    else:
        form = RouteStopForm()
